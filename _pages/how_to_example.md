---
title: User Guide Procedures and Concepts Sample
...
ProductAgent provides endpoint security and web activity monitoring through ProductSecurity. ProductAgent replaces the legacy SecurityAgent, which only supported proxy connections. 
...
4. Install ProductAgent on the endpoint. See the "Install ProductAgent on Windows Endpoints" procedure below.
...
3. The **Create Application Wizard** displays. Complete the following:
   1. On the **General Information** tab, enter the **Name** and additional fields, such as **Publisher** and **Administrator name**, then click **Next**.
...
4. The **Create Deployment Type Wizard** displays. Complete the following:
   ...
   3. On the **Content** tab, browse to the location of the ProductAgent installation package, specify the command used to install ProductAgent (`"C:\Program Files (x86)\<company_name>\Installer.exe" -x`), replacing <company_name> with the specific folder name, and browse to the location of the ProductAgent uninstallation package. Click **Next**.
   ...
   7. On the **User Experience** tab, select the following, then click **Next**:
...
10. View the deployment status at the bottom left pane of the Microsoft Endpoint Configuration Manager.
...
- **Proxy connect** mode: When ProductAgent is in proxy connect mode, ProductAgent redirects web traffic through the cloud proxy to the Internet. If the connection to the cloud proxy is unavailable, then ProductAgent falls back to the configured Fallback mode.
...
- **Auto-switch** mode: When ProductAgent is in auto-switch mode, ProductAgent starts in proxy connect mode and redirects web traffic through the cloud proxy to the Internet. 

   ProductAgent switches to direct connect mode if:

   - Connectivity to the cloud proxy is lost.
   - Proxy connection performance is degraded. ProductAgent checks the connection latency performance every 30 minutes and compares the speed of the proxy connection and the direct connection. If the proxy connection is 3 times slower than the direct connection, ProductAgent switches to direct connect mode.   

   ProductAgent switches back to proxy connect mode if:

   - Connectivity to the cloud proxy is restored.
   - Proxy connection performance improves. ProductAgent checks the connection latency performance every 30 minutes and compares the speed of the proxy connection and the direct connection. If the proxy connection performance is no longer 3 times slower than the direct connection performance, ProductAgent switches back to proxy connect mode.  
...
5.  Under **Endpoint Configuration**, select the connection mode:
 ...
 - **Proxy Connect**: When ProductAgent is in proxy connect mode, ProductAgent redirects web traffic through the cloud proxy to the Internet. If the connection to the cloud proxy is unavailable, then ProductAgentt falls back to the configured Fallback mode.
...
Next steps: Select the fallback mode for the connection.
...
The fallback mode is configured in the web policy in the ProductSecurity Console. The fallback mode is set per policy and different policies can have different fallback modes. 
